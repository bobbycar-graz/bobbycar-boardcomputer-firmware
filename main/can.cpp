...
void sendCanCommands()
{
  ...
  const Controller *front =
      (swap ? settings.controllerHardware.sendBackCanCmd : settings.controllerHardware.sendFrontCanCmd) ? (swap ? &controllers.back : &controllers.front) : nullptr;
  const Controller *back =
      (swap ? settings.controllerHardware.sendFrontCanCmd : settings.controllerHardware.sendBackCanCmd) ? (swap ? &controllers.front : &controllers.back) : nullptr;
