...
void fixCommonParams()
{
    ...
    {
        const auto x          = motors();
        const auto shouldBeep = std::all_of(std::begin(x), std::end(x), [](const bobbycar::protocol::serial::MotorState &motor)
                                            { return motor.pwm < 0; });
...
void readPotis()
{
    [[maybe_unused]] constexpr auto sampleMultipleTimes = [](int pin)
    {
        analogRead(pin);
        double sum {};
