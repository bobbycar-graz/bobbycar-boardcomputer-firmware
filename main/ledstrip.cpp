...
void updateLedStrip()
{
    ...
    else if ((cpputils::is_in(blinkAnimation, LEDSTRIP_OVERWRITE_BLINKLEFT, LEDSTRIP_OVERWRITE_BLINKRIGHT, LEDSTRIP_OVERWRITE_BLINKBOTH)) && settings.ledstrip.enableBeepWhenBlink)
        have_disabled_beeper = false;
...
void showAnimation()
{
    ...
    {
        if (animation_type == LedstripAnimation::DefaultRainbow)
            showDefaultLedstrip();
        else if (animation_type == LedstripAnimation::BetterRainbow)
            showBetterRainbow();
        else if (animation_type == LedstripAnimation::SpeedSync)
            showSpeedSyncAnimation();
        else if (animation_type == LedstripAnimation::CustomColor)
            showCustomColor();
        else
            showDefaultLedstrip();
    }
